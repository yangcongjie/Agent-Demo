import logging
import sys
from typing import Optional
from fastapi import FastAPI, Depends, HTTPException, Query, Header
from pydantic import BaseModel
import uvicorn

# 你的原有内部导入（路径不动）
from demo.logger_config import setup_logging
from demo.agent_runtime import AgentRuntime
from demo.mcp.start_mcp import start_mcp_proxy, stop_mcp_proxy

# ===================== 全局配置 =====================
logger = logging.getLogger(__name__)
# 外网访问密钥，自行修改一串复杂字符
API_ACCESS_TOKEN = "1125"
# 全局Agent实例
agent_instance: Optional[AgentRuntime] = None

# FastAPI 应用实例
app = FastAPI(
    title="杨聪杰的演示Demo",
    description="手动实现Agent Runtime，提供本地工具和高德地图MCP服务",
    version="1.0.0"
)

# ===================== 鉴权工具 =====================
def verify_access_token(x_token: str = Header(..., description="请求头携带Token")):
    if x_token != API_ACCESS_TOKEN:
        raise HTTPException(status_code=401, detail="Token 校验失败，禁止访问")
    return True

# ===================== 请求模型 =====================
class ChatPostRequest(BaseModel):
    query: str

# ===================== API 接口 =====================

# 2. POST接口：规范版本，长文本/复杂内容用
@app.post("/api/chat", dependencies=[Depends(verify_access_token)], summary="对话接口")
def chat_post_normal(req: ChatPostRequest):
    global agent_instance
    if agent_instance is None:
        raise HTTPException(status_code=500, detail="Agent 实例未初始化")

    reply = agent_instance.run(req.query)
    return {
        "user_query": req.query,
        "answer": reply,
        "trace_logs": agent_instance.trace
    }

# 健康检测接口
@app.get("/health", summary="服务健康检查")
def health_check():
    return {
        "service_status": "running",
        "agent_ready": agent_instance is not None
    }

# ===================== 两种启动模式函数 =====================
def run_console_interactive():
    """本地调试模式"""
    global agent_instance
    setup_logging()
    logger.info("=== 日志加载完成，进入控制台交互模式 ===")

    # 初始化Agent + MCP内部代理
    agent_instance = AgentRuntime()
    start_mcp_proxy()
    logger.info("Agent 已就绪，MCP中转代理启动完成")

    print("\n===== Agent 控制台交互 =====")
    print("输入提问进行对话，输入 quit / exit / q 退出程序\n")

    try:
        while True:
            user_input = input("用户：").strip()
            if not user_input:
                continue
            if user_input.lower() in ("quit", "exit", "q"):
                logger.info("退出")
                break

            logger.info(f"用户输入内容：{user_input}")
            result = agent_instance.run(user_input)
            print(f"\n智能助手：{result}\n")

            # 打印工具调用Trace日志
            if agent_instance.trace:
                print("----- 本次调用链路 Trace -----")
                for item in agent_instance.trace:
                    tool_name = item.get("tool", "")
                    args = item.get("args", {})
                    output = item.get("result") or item.get("error")
                    print(f"工具：{tool_name} | 参数：{args} | 返回：{output}")
                print("-" * 40 + "\n")

    finally:
        # 无论正常退出还是异常崩溃，都关闭MCP代理
        stop_mcp_proxy()
        logger.info("MCP中转代理已安全关闭，程序退出")


def run_fastapi_server(api_host: str = "0.0.0.0", api_port: int = 8000):
    """启动FastAPI接口服务（用于ngrok外网穿透）"""
    global agent_instance
    setup_logging()
    logger.info("=== 日志加载完成，准备启动API服务 ===")

    # 全局初始化一次Agent和MCP代理
    agent_instance = AgentRuntime()
    start_mcp_proxy()
    logger.info(f"Agent初始化完成，API服务监听 {api_host}:{api_port}")

    try:
        # 启动uvicorn服务
        uvicorn.run(
            app=app,
            host=api_host,
            port=api_port,
            reload=False,
            log_level="info"
        )
    finally:
        # 服务停止后销毁MCP代理
        stop_mcp_proxy()
        logger.info("API服务已停止，MCP代理关闭")

# ===================== 程序入口 =====================
if __name__ == "__main__":
    # 命令行参数解析
    # 使用示例：
    # 1. 默认控制台模式：python main.py
    # 2. API默认8000端口：python main.py --api
    # 3. API自定义端口8080：python main.py --api --port=8080
    args_list = sys.argv
    if len(args_list) >= 2 and args_list[1] == "--api":
        target_port = 8000
        # 解析端口参数
        for arg in args_list[2:]:
            if arg.startswith("--port="):
                target_port = int(arg.split("=")[1])
        # 修复：传参用 api_port=
        run_fastapi_server(api_port=target_port)
    else:
        run_console_interactive()